...
- **EDI export** — produces valid REG1TEST EDI files (one per band): correct `[REG1TEST;1]` header, `SPowe`/`SAnte`/`STXEq`/`SRXEq`/`SAntH` equipment fields, `PSect` category, full C* score summary block (`CQSOs`, `CQSOP`, `CWWLs`, `CWWLB`, `CExcs`, `CExcB`, `CDXCs`, `CDXCB`, `CToSc`, `CODXC`), and correct 15-field QSO records (dupe flag at col 14 per spec). Spec-compliant scoring: total score = sum of km (`CToSc` = `CQSOP`, no multipliers; claimed multipliers `;0;1`); duplicate QSO-points set to 0; output enforced to 7-bit ASCII (diacritics transliterated)
...
| `vhf-logger/vhf-logger.test.js` | 241 | 21 (`baseCall`, `normBand`, `locToLatLon`, `haversine`, `calcBearing`, `levenshtein`, `isDupe`, `recalcDupes`, `buildEdi`, `lookupCall`, `computeLogStats`, `toAscii`, `exportSlug`, `computeXFlags`, `sessionEdit`, `parseEdiForImport`, `makeZip`, `bandColors`, `manualTime`, `backup`, `I18N`) |
| `adif-stats.test.js` | 168 | 27 (`lookupCall`, `normBand`, `normMode`, `locToLatLon`, `haversine`, `parseADIF` ×3, `computeStats` ×7, `applyFilters`, `fmtDate`, `fmtMonth`, `htmlEsc`, `svgHBar`, `svgVBar`, `color helpers` ×4, `I18N`) |
...
- **EDI izvoz** — ustvari veljavne REG1TEST EDI datoteke (eno per pas): pravilna glava `[REG1TEST;1]`, polja opreme `SPowe`/`SAnte`/`STXEq`/`SRXEq`/`SAntH`, kategorija `PSect`, blok C* povzetka točkanja (`CQSOs`, `CQSOP`, `CWWLs`, `CWWLB`, `CExcs`, `CExcB`, `CDXCs`, `CDXCB`, `CToSc`, `CODXC`) in pravilni 15-polni zapisi QSO (zastavica duplikata na stolpcu 14 po specifikaciji). Spec-skladno točkanje: skupna ocena = vsota km (`CToSc` = `CQSOP`, brez multiplikatorjev; prijavljeni multiplikatorji `;0;1`); QSO-točke duplikata nastavljene na 0; izhod prisiljen na 7-bit ASCII (diakritični znaki transliterirani)
